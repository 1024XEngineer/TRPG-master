...
TRPG 内容常被混为一谈。本项目严格区分三者,它们决定了后面所有拆分:

| 概念 | 是什么 | 本项目中的形态 | 谁负责 |
|------|--------|----------------|--------|
| **规则(Rulebook)** | 游戏怎么"跑":检定、战斗、SAN 等机制 | 拆成 A/B/C 三类(见 §4) | 平台构建 |
| **设定(Setting/Lore)** | 世界"是什么样":生物、装备、术语、年代 | 属于 B/C 类(见 §4) | 平台构建 |
| **模组(Scenario)** | 一个具体"故事":地点、NPC、剧情 | 解析成剧本 IR,运行时读它 | 用户上传 + 官方 |

**类比:** 规则 = 象棋走子规则;设定 = 三国主题背景;模组 = 一盘具体棋谱。

**两个关键推论:**
- **COC 规则 ≠ 克苏鲁设定。** COC 是规则系统(d100/SAN),通常但不必然搭配克苏鲁神话。模组可用 COC 规则却是其他题材。
- **模组是自包含的。** DM 运行时只需"该模组 IR + 引擎结果 + 少量按需检索的设定",**不需要整本规则书喂给 LLM**。
