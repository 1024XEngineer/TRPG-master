...
4. [`docs/current/README.md`](docs/current/README.md)：仍在维护的策略、成员流程、实施计划和术语文档索引。

`docs/architecture.md` 和 `docs/数据模型设计.md` 是根级规范文档；`docs/current/` 是现行扩展说明，不能覆盖根级规范和代码事实源。`docs/archive/` 仅用于追溯讨论，不是实现依据。发生冲突时，以 Pydantic/Protocol、自动生成的 JSON Schema 和根级规范文档为准。
