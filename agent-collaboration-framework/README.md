...
assemble_context -> interpret
                       ...
                       |-- route=none -------> narrate -> summary_op -> END
                       `-- module/default ---> engine_node
                                                  |
                                                  `-> narrate -> summary_op -> END
```
...
[`ports.py`](collaboration_framework/ports.py) 不导入 LangGraph。四个公共 Protocol 是：
...
`schemas/` 由 Pydantic Model 自动生成，不应手改。
