...
```text
assemble_context -> interpret + harden_intent_routing
...
`Intent.execution` 和 `Intent.check` 都是解释器提议。`interpret_node` 使用可信
...
[`routing.py`](collaboration_framework/routing.py) 不导入 LangGraph。五个公共 Protocol 是：
...
[`workflow.py`](collaboration_framework/workflow.py) 只是这些接口与确定性路由策略的薄包装。
替换编排器时，各组件实现、路由策略和 JSON 契约不需要改变。
