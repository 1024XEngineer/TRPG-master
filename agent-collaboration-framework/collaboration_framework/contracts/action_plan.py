...
ActionPlanStepKind: TypeAlias = Literal[  # noqa: UP040
    "travel",
    "wait",
    "rest",
    "action",
...
class ActionPlan(ContractModel):
    ...
    steps: tuple[ActionPlanStep, ...] = Field(min_length=2)
...
class ActionPlanPolicy(ContractModel):
    ...
    def validate_window(self) -> ActionPlanPolicy:
        if self.max_steps_per_advance > self.max_plan_steps:
            raise ValueError("max_steps_per_advance 不能超过 max_plan_steps")
        return self
...
class ActionPlanProgressEvent(ContractModel):
    ...
    def validate_counts(self) -> ActionPlanProgressEvent:
        if self.current_step > self.total_steps or self.completed_steps > self.total_steps:
