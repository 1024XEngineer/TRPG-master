...
选择：`ModuleContent`、声明式 `CheckpointSpec` 和 `RuleSpec` 放在 `contracts/module.py`，由 B/C 共同评审。C 的 `ModuleDraft` 是 Parser 私有 IR，不从共享 `contracts/` 或 `collaboration_framework.module` 包级接口导出。C 的 `module/validation.py` 是从 Draft 语义校验并构造 `ModuleContent` 的唯一入口；B 只消费 `ModuleContent` 执行规则；A 不消费二者。

理由：`ModuleContent` 是 C 到 B 的发布协议。若留在 C 私有目录，B 只能依赖 C 实现；若放进 B 内部目录，C 的输出侧只能反向依赖运行时。公共契约解决的是“模组能声明什么”，不包含 Parser 中间态或“引擎怎样执行”。Phase 2 Parser 输出 `ModuleDraft`，不得直接构造 `ModuleContent`；Validation 不承担 Parser、Review、Publish 或 Runtime 职责。

## 4. ModuleContent 的三层边界

```mermaid
flowchart LR
    RAW["Raw JSON / LLM Output"] --> DRAFT["C 私有 Parser IR<br/>ModuleDraft"]
    DRAFT --> VALIDATE["C Validation<br/>语义校验 + 唯一转换入口"]
    VALIDATE --> CONTRACT["contracts/module.py<br/>ModuleContent + 声明式 Specs"]
...
- C 私有：原文、`ModuleDraft`、证据位置、审查报告、人工批准流程。Draft 只能由 Parser 产生、由 Validation 消费。
- B/C 公共：发布后模组的结构、引用关系和声明式条件/操作。
- B 私有：编译后的 Rule、Hook 注册表、随机源、状态变更、Event、事务和缓存。
- A 私有：玩家可见投影政策、意图/叙事上下文、回合工作状态和 Gateway DTO。

依赖方向固定为：`Raw JSON / LLM Output → ModuleDraft → Validation → ModuleContent → Runtime`。Runtime 和其他共享模块不得引用 `ModuleDraft`；Validation 成功前不得构造供 Runtime 消费的正式 `ModuleContent`。
