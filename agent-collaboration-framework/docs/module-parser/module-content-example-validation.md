...
> **版本**：ModuleContent v1
> **验证日期**：2026-07-24
> **验证阶段**：发布 Schema 覆盖验证；Runtime 消费能力另行标注
> **锚定文档**：[module-content-field-decisions.md](./module-content-field-decisions.md) §8 P2 最终完整形状
>
> 当前 B/C 发布接口以 `collaboration_framework/contracts/module.py` 及生成的
> `schemas/module-content.schema.json` 为准；文中的 P0/P1/P2 是设计来源标签。
> 声明可通过 Schema 不代表占位 `RuleKernel` 已实现对应执行语义。

---

## 一、验证目标

验证 ModuleContent v1 是否能够表达真实 CoC 模组的内容，使用 1 个真实模组进行完整 Mapping：

1. **《幸福蛙蛙村》**（DOCX，12374 字）——短模组，含多导入、软判据、累积变异 Track、多结局、SAN 检定、战斗

验证方法：将模组原文逐段映射到样例实际使用的 5 个顶层集合（发布契约共 6 个，
本样例未使用 `information_items`）+ 四张声明表（Hook × Expression × Op × 内建变量），
记录可直接表达的内容和无法表达的内容（Gap），并按 Parser/Runtime/Host Agent/Ruleset/Future Capability 分层分类。
...
幸福蛙蛙村（~127 个可结构化机制）在当前 v1 发布设计下实现 ~100% L1 原文保存覆盖。所有机制均可通过样例实际使用的 5 个顶层集合（发布契约共 6 个）+ 四张表表达：
...
| Future Capability | 4 | ❌ 对应 Runtime 能力落地后验证 |
...
**ModuleContent v1 发布设计通过单模组概念覆盖验证。** 幸福蛙蛙村的全部
~127 个结构化机制均可被样例实际使用的 5 个顶层集合 + 四张表
（Hook × Expression × Op × 内建变量）覆盖，不新增领域对象。声明字段已进入当前
B/C Contract；对应执行语义是否可用仍取决于 Runtime 消费能力。
...
| 本验证报告 | [module-content-example-validation.md](./module-content-example-validation.md) |
