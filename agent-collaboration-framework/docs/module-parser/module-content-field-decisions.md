...
> **Status：B/C Published Contract（Runtime Consumer Partial）**
> **Version：ModuleContent v1**
> 日期：2026-07-23
> 锚定代码：`collaboration_framework/contracts/module.py`
>
> B/C 发布接口以锚定代码及生成的 `schemas/module-content.schema.json` 为唯一事实源。
> 本文中的 P0/P1/P2 是设计形成过程的分期标签；§8 的声明形状已经进入当前契约。
> Engine Service/Store、Host 与 Backend 的接口仍以 Agent 架构分支为准。
>
> **当前架构方向**：不使用 16 个独立集合（Track/Timeline/Encounter/Faction/EntryPoint 等）。通过扩展现有 `RuleSpec` 的四张表（Hook × Expression × Op × 内建变量），用组合方式覆盖模组机制。发布目标包含 6 个集合（4 个必需集合 + 2 个默认空集合）。

---

## 一、当前 ModuleContent 核心结构
...
class ModuleContent(ContractModel):
    ...
    background: str         # 时代、故事前提与叙事基调；每次叙述调用都会携带
...
破坏性重命名（Scene→ContentUnit, WinCondition→Ending）仍进入下一个 Contract major version。

---

## 二、P0：立即可以改的（4 处，~10 行，不 break 测试）

> 本章节保留设计演进和决策依据；§8 的声明形状现已成为当前 B/C 发布契约。
...
### 9. InformationItem（事实本体）+ ModuleContent.information_items + EntitySpec.information_item_ids

**决策**：新增 `InformationItem` 作为可稳定引用的事实本体（P1，从 Defer 移入）；`EntitySpec` 新增 `information_item_ids: tuple[str, ...] = ()`，与 `secrets` 并列，用于结构化引用该实体关联的 `InformationItem`。

**职责边界**：

| 字段 | 负责 | 不负责 |
|------|------|--------|
| `InformationItem` | 模组中可引用事实的静态声明——稳定 ID + content + 默认 visibility；被 Rule/Condition/Outcome/Entity 引用 | 信息获取过程（`InformationAcquisition`，Defer）、玩家知识状态（`KnowledgeState`，Defer）、Projection 动态过滤 |
| `EntitySpec.secrets` | 该实体独有、不需要被其他地方引用的隐藏状态/机制细节（自然语言） | 复述已被结构化为 `InformationItem` 的事实正文 |
| `EntitySpec.information_item_ids` | 结构化指向该实体关联的 `InformationItem`，供 Rule Engine/Validation 按 id 查找、Host Agent 按 id 跳转 | 事实内容本身——正文只存在于 `InformationItem.content`，不重复 |
...
class EntitySpec(ContractModel):
    ...
    information_item_ids: tuple[str, ...] = ()      # 结构化引用 InformationItem

class ModuleContent(ContractModel):
    information_items: tuple[InformationItem, ...] = ()
```

`ModuleContent.validate_references()` 校验 `entity.information_item_ids` 中的每个 id 必须存在于 `information_items` 集合，悬空引用直接拒绝发布，不静默忽略。

**消费者**：Rule Engine（Condition/Effect 按 id 解析引用的事实）、Validation（校验引用存在、id 唯一）、Host Agent（按 ID 查找事实正文/跳转定位）。

**不影响 Runtime**：不要求 B 实现授予机制、不要求 `KnowledgeState`。`CheckpointOutcomeSpec.facts` 仍然保留（裸字符串兼容），后续可迁移为 `information_item_ids` 引用。

**与 InformationAcquisition 和 KnowledgeState 的区别**：

| 概念 | 状态 | 原因 |
|------|------|------|
| `InformationItem` / `EntitySpec.information_item_ids` | **P1，已纳入** | 纯静态声明 + 结构化引用，不需要额外 Runtime 授予机制 |
| `InformationAcquisition` | Deferred | 需要 Runtime 授予机制 |
| `KnowledgeState` | Deferred | 属于 GameState，需要 Host Projection |

锚定代码：`collaboration_framework/contracts/module.py` 的 `EntitySpec`、`InformationItem`、`ModuleContent.validate_references()`；`schemas/module-content.schema.json` 已重新生成。
...
```python
# ===========================================================================
# ModuleContent 顶层（候选 6 个集合：当前 4 个 + P1 的 module_rules + information_items）
# ===========================================================================
...
class ModuleContent(ContractModel):
    ...
    background: str                                  # 模组级叙述上下文；不得绕过可见事实边界
    ...
    entities: tuple[EntitySpec]                      # 实体列表。EntitySpec: id, kind(npc|object|location),
                                                     #   name, aliases, content, secrets,
                                                     #   information_item_ids (P1), state,
...
class EntitySpec(ContractModel):
    ...
    content: str
    secrets: str | None = None
    information_item_ids: tuple[str, ...] = ()                # P1（见 §3.9）
...
## Published Contract Freeze Statement

ModuleContent v1 已作为 B/C 发布契约落地。当前 `RuleKernel` 仅是部分消费者，未支持的
Hook、Expression 或 Operation 应记为 Runtime Gap；后续问题优先按以下分类处理，不直接修改 Contract：
