...
flowchart TB
    subgraph INPUT["执行入口"]
        INTENT["接收 execution=engine 的 Intent<br/>与可信执行上下文"]
...
1. `AtomicActionEngine` 只接收 `intent.execution=engine` 的 `EngineRequest`：其中 `player_input` 携带可信 `room_id/player_id/actor_id/client_action_id`，`intent` 是必须重新复核的动作提议。
...
4. 幂等键已执行时直接返回完整的先前 `ActionResult`，其中保留原 Event 引用；不重复掷骰、写 Event 或触发 Rule。
...
1. `check.route=none` 不主动掷骰，但仍进入规则流水线，因为 `execution=engine` 已决定调用引擎，动作仍可能触发机制 A、B 或 D。
2. `check.route=module` 使用经过 Scene 复核的 Checkpoint。
3. `check.route=default` 使用 World 明确定义的默认 Checkpoint/检定机制。
...
当所有候选 Op 均通过校验后，AtomicActionEngine 开始单一数据库事务：
...
| 字段 | 引擎复核规则 |
| --- | --- |
| `execution` | `EngineRequest` 中必须为 `engine`；`narrative` 不得进入本端口 |
...
| 重复 `client_action_id` | 返回完全相同的先前 `ActionResult` 和原 Event 引用 | 不重复掷骰、触发、写 Event 或修改状态 |
...
| 状态仓储 | `packages/core/state/repo.py` | 当前保留 `load/save` 模式，由 Orchestrator 调用 `save`；目标是把写入收口到 AtomicActionEngine 事务 |
...
- 测试期间通过 `FakeAtomicEngine` 让双方并行，不让 A 依赖 B 的内部类。

### 9.2 成员 B → 成员 A

- 提供稳定 `AtomicActionEngine` 接口和完整 `ActionResult`。
...
1. 冻结 `EngineRequest`、`AtomicActionEngine`、`ActionResult`、`Event` 和有限 Op 契约。
...
- 同一幂等动作重放返回完全相同的 `ActionResult` 和原 Event 引用，同时不重复掷骰、触发 Hook、写 Event 或修改状态。
...
- **我向谁提供什么**：向成员 A 提供 `AtomicActionEngine/ActionResult`；向回放和审计提供 Event；向成员 C 提供可执行规则和 Schema 反馈。
