...
| `NarrationContext` | `player_input`, `intent`, `action_result`, `player_view`, `recent_history` | Narrator 使用 B 的安全结果、动作后视图和同一份近期历史选择结果 |
...
| 模型 | 字段 | 语义 |
|---|---|---|
| `HostAgentContext` | `player_input`, `player_view`, `recent_history` | 只接收可信 `PlayerInput`、player-safe `PlayerView` 与同作用域近期历史；`room_id/player_id/actor_id` 必须逐项一致 |
