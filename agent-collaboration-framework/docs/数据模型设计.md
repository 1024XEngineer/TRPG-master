...
- `ActorState`: `player_id`, `name`, `source_character_id`, `source_character_version`, `state`, `resources`, `conditions`, `condition_states`, `sanity`
...
- `EngineRuntimeSnapshot`: `module_id`, `module_version`, `module_content`, `game_state`, `revision`, `event_history`
...
- `ModuleContentV3`: `content_schema_version`, `module_id`, `version`, `world_ref`, `background`, `opening_text`, `opening_key_facts`, `information`, `knowledge_goals`, `entities`, `locations`, `location_edges`, `rules`, `core_resolution`, `ending_policy`, `ending_anchors`, `presentation`, `initial_state`, `world_profile`, `time_policy`, `sanity_sources`
...
ActorState.sanity 保存私有规范损失账本、稳定来源习惯化和历史覆盖范围；ModuleContentV3.sanity_sources 注册来源。EngineRuntimeSnapshot.event_history 仅用于旧快照从规范事件补账，不进入玩家投影。

PR3：ActorCondition.details 保存已抽样的安全机械事实，ConditionExpiry.absolute_hour 固定小时到期；CheckConsequenceParent 连同 consequence_spec/parent 保存追加检定的原始父结果。sanity_policy 选择概括发作或明确未实现的按轮模式。ProjectionSelfActor / SelfActorView 的 condition_details 仅含安全标签及相对 remaining_hours，不含来源或绝对时钟。
