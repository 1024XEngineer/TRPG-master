...
| `visible_facts` | `tuple[VisibleFact, ...]` | 已由 B 按发现状态与受众策略过滤的事实 |
...
| `discovered_facts` | 已由规则确认发现的 `InformationItem` ID |
...
| `SceneSpec` | `id`, `name`, `content`, `entity_ids`, `checkpoint_ids`, `exits` |
| `StatBlock` | CoC 属性、HP/MP/MOV、`armor`, `move` |
| `EntitySpec` | `id`, `kind`, `name`, `aliases`, `content`, `secrets`, `information_item_ids`, `state`, `refuse_ops`, `blocked_text`, `direct_responses`, `rules`, `stat_block` |
...
| `InformationItem` | `id`, `content`, `visibility` |
