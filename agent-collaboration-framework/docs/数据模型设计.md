...
> 本文记录当前代码已经实现的数据模型，不代表多模组统一目标模型。统一扩展方向与候选字段的四问评估见 [`architecture/data-model-alignment-rfc.md`](architecture/data-model-alignment-rfc.md)。该 RFC 在团队冻结并落地前，不改变 `contracts/module.py` 的现行权威性。
