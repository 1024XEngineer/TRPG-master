...
| `HostAgentContext` / ActionPlan Run | Host | 数据库 session、Provider SDK、未来步骤效果 |
...
`KeeperCapabilityView` 只进入 Planner 和当前步骤 adjudicator。它可以提供 Canon/Runtime
information、location、entity、ending 与 world profile 的受控词汇，但不直接授权任何效果。
Engine 必须基于提交时的同一 runtime snapshot 重新校验每个引用。

## 3. 当前 Host 模型

### 3.1 Planner

`HostAgentContext` 包含 `player_input`、`player_view`、`recent_history` 和可选
`keeper_capabilities`。构造时强制校验：

- input/view 的 room、player、actor 完全相同；
- RecentHistory 属于相同 room/viewer 且不晚于当前 revision；
- Keeper capability 属于相同 room/actor/revision；
- 玩家安全视图和 Keeper 词汇不能跨作用域拼接。

Provider 返回的决策候选由 `HostTurnDecisionParser` 校验。`SingleActionDecision` 直接包含
当前 `ActionAdjudication`；`ActionPlan` 的每个 step 只保存 `kind`、`semantic_goal` 与可选
安全进度标签，不保存未来效果、隐藏信息、目标 ID 或 revision。
...
`ModuleContent` 是 C 发布、B 执行的跨成员协议。顶层包含 module/version、world/background、
presentation、初始场景、scenes、entities、checkpoints、win conditions、module/event rules 和
information items。

`ConditionSpec` 支持结构化 path/equals 或受控表达式；`RuleSpec` 定义 hook、priority、mode、
condition、operations 和玩家安全信息；`CheckpointSpec` 把 scene/action/target/skills/difficulty、
声明候选、outcomes 和 visibility 绑定在一起。所有引用在发布时验证，运行时仍由 Engine 复核。
...
Framework 发布 `HostAgentContext`、ActionPlan/Adjudication、Opening、Narration、RecentHistory、
PlayerView 和 Module Schema。backend 的 WebSocket DTO、REST DTO 和 `trpg-sdk` 生成类型有独立
事实源；Framework Host 内部模型不能替代或自动改写传输协议。

## 8. 当前模型字段索引

本节用于架构漂移测试，字段名与 Pydantic `model_fields` 保持同步。

- `PlayerInput`: `room_id`, `player_id`, `actor_id`, `client_action_id`, `utterance`
...
- `ConditionSpec`: `path`, `equals`, `expr`
- `AllowOperationSpec`: `op`, `action`
- `ModifyOperationSpec`: `op`, `path`, `set`
- `RuleSpec`: `id`, `hook`, `priority`, `mode`, `when`, `then`, `facts`, `discover_information_ids`, `player_visible_information`
- `SceneSpec`: `id`, `name`, `content`, `player_visible_name`, `player_visible_description`, `narrative_details`, `entity_ids`, `checkpoint_ids`, `exits`, `available_exits`
- `EntitySpec`: `id`, `kind`, `name`, `aliases`, `player_visible_name`, `player_visible_aliases`, `content`, `narrative_details`, `visibility`, `observable_state`, `secrets`, `information_item_ids`, `state`, `refuse_ops`, `blocked_text`, `direct_responses`, `rules`, `stat_block`
- `CheckpointOutcomeSpec`: `facts`, `discover_information_ids`, `player_visible_information`, `narration_constraints`, `ops`
- `CheckpointOutcomesSpec`: `success`, `failure`, `critical_success`, `extreme_success`, `hard_success`, `regular_success`, `fumble`
- `CheckpointSpec`: `id`, `scene_id`, `action`, `target_id`, `skills`, `difficulty`, `declaration_options`, `outcomes`, `visibility`
- `WinConditionSpec`: `id`, `when`, `fact`, `player_visible_information`, `is_ending`
- `ModuleContent`: `module_id`, `version`, `world_ref`, `background`, `presentation`, `initial_scene_id`, `scenes`, `entities`, `checkpoints`, `win_conditions`, `module_rules`, `event_rules`, `information_items`
