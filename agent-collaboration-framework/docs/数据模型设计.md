...
`ProjectionEntity` 只含 `id/kind/name/aliases/description/observable_state`。其中
`observable_state` 逐项来自 `EntitySpec.observable_state` allow-list，不复制原始 Entity state。

`ProjectionCheckpointOption` 只含 `id/target_id/action_hint/skills/difficulty`。`action_hint` 是语义提示，不是封闭动词白名单。
...
| `scene` | `SceneView` | 场景安全名称/描述/时间及 `visible_entities`、`visible_actors`、`available_exits` |
...
| `declarations` | `tuple[str, ...]` | 玩家明确声明的规则要素，例如“屏住呼吸” |
...
| `entities` | 当前权威实体状态 |
| `clock` | `ClockState`，保存累计分钟、昼夜和回合数 |
...
`ActorResources` 与 `ClockState` 都是可序列化的确定性状态；随机检定、资源变化和时间推进必须先写入
`GameState` 并产生 Event，Agent 不得在提示词中自行维护这些值。
...
顶层字段为
`module_id/version/world_ref/background/presentation/scenes/entities/checkpoints/win_conditions/module_rules/information_items`。
...
| `RuleSpec` | `id`, `hook`, `priority`, `mode`, `when`, `then`, `facts`, `player_visible_information` |
...
| `EntitySpec` | `id`, `kind`, `name`, `aliases`, `content`, `visibility`, `observable_state`, `secrets`, `information_item_ids`, `state`, `refuse_ops`, `blocked_text`, `direct_responses`, `rules`, `stat_block` |
