...
`mvp_check_result` 已移出生产 Contract。当前占位 `RuleKernel` 通过注入式
`check_outcome_resolver` 控制测试结果；生产检定结果仍必须由 B 的权威规则/随机源产生。
...
占位 `RuleKernel` 只执行当前纵切需要的等值条件、交互 Hook 和少量状态 Operation；
其他 Hook、表达式和 Operation 可以合法发布，但若占位内核实际走到尚未实现的执行分支会明确拒绝。
