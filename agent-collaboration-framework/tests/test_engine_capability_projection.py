...
class EngineCapabilityProjectionTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_world_time_is_projected_as_the_module_declared_wording(self) -> None:
        """玩家侧只拿得到措辞；精确时刻是 Keeper capability 的事（#415）。"""

        snapshot = await self.engine.read(SCOPE)
        self.assertEqual(snapshot.world.time_label, "下午")
        # 反向断言：可反推出小时的字段一个都不能留在玩家侧。
        for field in ("day_index", "hour_of_day", "time_of_day"):
