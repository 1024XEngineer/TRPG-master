...
from collaboration_framework.engine import GameState, RuleKernel
...
def actor_state(player_id: str) -> dict:
    return {
        "player_id": player_id,
        "name": "调查员",
        "source_character_id": f"character:{player_id}",
...
class ModuleCapabilityTests(unittest.TestCase):
    def test_non_terminal_win_condition_does_not_write_ending_state(self) -> None:
        payload = json.loads(
            (ROOT / "fixtures" / "demo-module.json").read_text(
                encoding="utf-8"
            )
        )
        payload["win_conditions"].append(
            {
                "id": "milestone_key_found",
                "when": {
                    "path": "entities.bookshelf.key_found",
                    "equals": True,
                },
                "fact": "玩家找到了钥匙",
                "player_visible_information": "这是一项进展，但不是结局。",
                "is_ending": False,
            }
        )
        report = validate_module_json(json.dumps(payload))
        ...
        application = build_fake_application(
            ...
            GameState.model_validate(
                {
                    "room_id": "milestone_room",
                    "scene_id": "study",
                    "actors": {
                        "investigator": actor_state("milestone_player")
                    },
                    "entities": {
        ...
        player_input = PlayerInput(
            room_id="milestone_room",
            player_id="milestone_player",
            actor_id="investigator",
            client_action_id="find_key_milestone_001",
            utterance="我仔细调查书架。",
        )

        output = asyncio.run(application.orchestrator.run(player_input))

        final_state = application.engine_store.inspect_state("milestone_room")
        execution = application.engine_store.inspect_completed_action(
            "milestone_room",
            "find_key_milestone_001",
        ).execution
        self.assertEqual(final_state.phase, "playing")
        self.assertIsNone(final_state.ending_id)
        self.assertEqual(
            [event.payload.path for event in execution.events],
    ...
    def test_action_does_not_execute_on_scene_enter_rule(self) -> None:
        """The placeholder action path must not consume another hook."""
        ...
        initial_state = GameState.model_validate(
            {
                ...
                "scene_id": "study",
                "actors": {
                    "investigator": actor_state("hook_audit_player")
                },
                "entities": {
                    "cabinet": {
                        "scene_ready": True,
        ...
        final_state = application.engine_store.inspect_state("hook_audit_room")
        execution = application.engine_store.inspect_completed_action(
            "hook_audit_room",
            "hook_audit_open_001",
        ).execution
    ...
    def test_checkpoint_failure_consumes_all_outcome_fields(self) -> None:
        ...
        initial_state = GameState.model_validate(
            {
                ...
                "scene_id": "study",
                "actors": {
                    "investigator": actor_state("failure_player")
                },
        ...
        application = build_fake_application(
            ...
            rule_kernel=RuleKernel(
                lambda _request, _checkpoint: "failure"
            ),
        )
        ...
        final_state = application.engine_store.inspect_state("failure_room")
    ...
    def test_rule_allows_action_after_checkpoint_changes_state(self) -> None:
        ...
        initial_state = GameState.model_validate(
            {
                ...
                "scene_id": "study",
                "actors": {
                    "investigator": actor_state("capability_player")
                },
                "entities": {
                    "bookshelf": {"key_found": False},
        ...
        final_state = application.engine_store.inspect_state("capability_room")
        open_execution = application.engine_store.inspect_completed_action(
            "capability_room",
            "open_cabinet_001",
        ).execution
