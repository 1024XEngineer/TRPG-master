...
- Issue #406 / PR #407：本文档的设计评审载体，不要求先合并，也不再新建总 Issue；后续
  五个实现 PR 直接引用 #407，只有某个 PR 需要独立延期、换负责人或单独追踪时才补 Issue。
...
如果同一个普通 Keeper 回合既需要守秘人叙事、又需要多个 NPC 说话，这不属于 PR2 的纯
`@NPC` 对话路径，而属于后续“守秘人 / NPC 同回合双回复编排”PR。该场景的固定时序是：

```text
先提交 Keeper narration.push
再按 ordinal 依次提交 dialogue.npc(1..N)
```

任一 NPC 回复失败都不能回滚已经提交的 Keeper 权威结果；最多只允许补一条澄清或直接跳过
该 NPC 回复，避免把房间主链卡死。
...
13. **同回合编排边界**：纯 `@NPC` 路径和 Keeper 行动路径在 PR2 中保持分离，不在一个队列
    项里混合生成守秘人结果和 NPC 台词；普通 Keeper 回合中的“守秘人 + 多 NPC 连续发言”
    由后续 PR3 单独实现，顺序固定为 `narration.push -> dialogue.npc(1..N)`。
14. **故障恢复边界**：事件受众和顺序必须与权威 Event 同事务提交，不能出现一部分玩家看到
    Keeper 回复、另一部分玩家只看到 NPC 回复的半套状态；若任一步骤无法保证完整提交，
    则回退为稍后重试，而不是广播不完整结果。
...
PR #404 已经合并，五个实现 PR 都从当时最新 `main` 顺序开发。每个 PR 必须包含自己的
...
### 12.3 PR3：守秘人 / NPC 同回合双回复编排

工作内容：

- 在现有 Keeper `ActionPlan` 结果之上增加结构化“同回合回复编排”，允许一次玩家提交产出
  `narration.push + dialogue.npc` 的连续权威消息；
- 复用 PR2 的 `dialogue.npc` 持久化契约，不再新增第二套 NPC 消息协议；
- 将“场景描述 / 行动结果 / 世界状态变化”严格保留给 Keeper narration，把 NPC 原话从
  Narrator 自由文本中拆出为独立 `dialogue.npc`；
- 在 Planner / Narrator 契约中显式增加“本回合是否需要 NPC 回复、回复哪些 NPC、回复顺序”
  的结构化槽位，不靠前端或后处理猜测文本；
- 保持 Engine 仍是唯一事实源：NPC 回复只表达认知和台词，不直接确认旅行、检定、物品变化
  或线索发现；
- 增加“守秘人先叙事、NPC 后发言”的前端连续展示与刷新恢复测试；
- 覆盖“普通行动不涉及 NPC 时仍只有 Keeper 回复”“多个 NPC 同回合按顺序回复”“NPC
  回复失败时只保留 Keeper 权威结果或澄清”的运行时测试。

边界：本 PR 不新增 NPC 长期 Memory、Summary、历史搜索或摘要任务；只解决当前“守秘人把
NPC 台词埋在 narration.push 里”的编排和展示问题。若玩家显式 `@NPC`，仍继续走 PR2 的纯
NPC 对话路径，不与 Keeper 行动路径混用。

验收：当一次普通 Keeper 行动既需要守秘人叙事、又需要 NPC 说话时，界面展示为两条连续
消息：先 `守秘人`，后 `NPC`；刷新和回放后顺序不变；NPC 台词不再埋入守秘人气泡；没有
NPC 参与的回合行为保持现状。
...
PR3
  无强制迁移；优先复用 PR2 的 dialogue Event。
  只有在 Keeper / Narrator 结构化编排契约落库确有必要时，才增加最小字段。
...
- 不扩大已合并的 PR #393；PR #404 已经进入 `main`，是五个实现 PR 的共同基础；
- PR #407 是设计基线，不要求先合并；五个实现 PR 都在描述中引用 #407；
- 默认按 PR1 → PR2 → PR3 → PR4 → PR5 顺序合并，每个分支从包含前一项的最新 `main` 创建；
- 若团队需要并行开发，可以临时使用 stacked PR，但前一项合并后必须 rebase 到最新 `main`
  并重新运行完整 CI，不能长期依赖未合并分支；
- 用户从 PR2 开始可以试玩显式 `@NPC` 对话；验证“守秘人 + NPC 双回复”时测试 PR3；验证
  长期记忆时测试 PR4，验证完整长团能力时只测试最新 PR5；
- 不为五个 PR 分别创建 Issue。只有某个 PR 被独立延期、换负责人、需要不同发布周期或出现
  超出本文范围的新需求时，才单独补 Issue；
- PR1 和 PR2 默认不合并为一个大 PR，因为路由回归与 Host 运行时故障面不同；若 PR1 实际
  只剩很小的契约改动，可以在开发前并入 PR2，但不能省略 PR1 的独立验收项；
- PR3 和 PR4 默认分开，避免把“同回合双回复编排”错误和长期记忆召回错误混在一起排查；
- PR4 和 PR5 默认分开，避免把基础召回错误和摘要压缩错误混在一起排查。
...
- [x] 实现按 PR1～PR5 顺序交付，不为每个 PR 预先创建 Issue；只有独立延期、换负责人或
  超出本文范围时才补 Issue。
