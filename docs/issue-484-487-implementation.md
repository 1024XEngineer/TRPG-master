...
基线 `98a9aba5357fba507047ad9dd1f478d7f27cfc52`。按用户 2026-09-13 的最新要求，四阶段均进入同一条本地分支 `feat/issue-484-487-capability-completion`。四阶段先在独立工作区实现和验证，随后按用户要求部署、推送并创建 [PR #532](https://github.com/1024XEngineer/TRPG-master/pull/532)。下文保留各阶段验证记录；最新时间机制调整见末节。
...
临时状态与概括发作分别抽取并保存期限。按后续体验修订，小时期限使用 `absolute_hour`，不注册中途TimeTask；实际时间推进到正常/剧情时间点后，统一移除截止时刻不晚于当前世界时间的活动条件，再继续规则。新的每日time_point引用只绑定一次绝对日期；旧引用保留“下次匹配点”兼容语义。安全休息和发作中断由规则拥有的动作完成，提前解除后的期限不再结算。结束疯狂不清除模组另加的昏迷。
...
按轮模式明确报INSANITY_ROUNDS_UNSUPPORTED，已提交SAN保持唯一；机制期限可以晚于模组终点，抵达终点仍未到期的状态保留，不拒绝合法的SAN后果，也不延长剧情时间线。恢复使用原抽样依据，事务失败不会留下半份Condition。
...
新房间初始化起始基准，SAN增加不移动该基准；规范损失在来源封顶/下界约束后才累计。明确支持Keeper安全休息窗口和作者声明的世界日窗口。旧窗迟到/重复事实不污染当前窗，两个actor分别累计。跨日和安全休息不会解除不定性疯狂；同次单次与累计阈值优先不定性；已有临时状态升级时移除旧状态，其小时期限不再结算。

月度治疗由 `coc7.start_treatment` 和 `coc7.review_treatment` 执行。显式公历锚点与持久化疗程决定复查截止小时；每次进入世界时间点后检查截止时间，不插入机制时间点。到期仍保留疯狂，只开放检查，并持久化单次提醒标记。私护/机构、成功后SAN恢复检查、无进展、恶化停月、外来创伤中断、新疗程和发展阶段明确恢复均有消费者。资源增减复用通用Effect，治疗SAN收益上限由CoC7计算后交给通用执行器，保留原骰面及请求/实际值。
...
实现范围仍不包含E6b按轮期限、永久疯狂/退场、完整战斗或发作叙事动作；缺少真实回合能力时明确报错。本次PR不自动关闭Issue；自动化验证不等同于人工规则复核。
...
原计划文件保留为规划基线，本文是实施记录。实际同步完成：本地分支 `feat/issue-484-487-capability-completion` 包含四个顺序检查点，前三个为 `a0a9b6b`、`90b1771`、`0212bd3`，第四个为本节所属提交。实现仍集中在同一分支，隔离工作区使用detached HEAD；后续已按用户要求推送并创建PR #532。
...
## PR #532 后续调整：机制期限随大块时间推进结算

用户确认的体验：机制状态不打断大块时间推进；只有剧情任务注册中途时间点。

- 主红测：12:00发生临时疯狂，期限15:00、发作期限14:00，旧实现下一跳落在14:00而非18:00。修正后一次推进到18:00，两个状态各解除一次，没有机制TimeTask或临时occurrence。
- 临时疯狂、概括发作保存绝对小时截止时间，条件消费者在 `time.point_entered` 时使用 `deadline <= 当前世界小时` 判断。跨日、恰好到期、一次跨越多项期限均有效；提前解除和请求重放不会重复移除。不定性疯狂仍无自动到期时间。
- 月度治疗继续使用显式公历锚点。世界时间进入截止点之后的正常时间点时，只发一次 `actor.treatment_review_due`；`review_due_notified` 与事件同事务保存。复查动作直接校验疗程、月份和当前世界时间，恢复仍须通过原有治疗/SAN检定。下一周期重置提醒标记。
- 同日延后几个小时处理复查，下一截止时刻会满足“至少再过一个公历月”（恶化为两个），不会因小时差多跳过整月。真正错过日期的晚复查继续按疗程月边界安排，不补掷历史月份。
- 剧情TimeTask继续插点、到期并定向交给作者规则。同小时若还有剧情任务，机制期限迁移或失效不会移除该剧情停靠；hidden剧情任务也保留该行为。
- 旧版PR存档读取时，将角色条件/疗程明确关联的内部任务转成机制期限，保留已完成记录；尚未到期的内部任务标记cancelled及迁移原因，收回仅被其占用的临时点。转换幂等，不修改历史事件或SAN账本，不需要新增数据库迁移。旧版本代码不能读取新增的小时期限类型。
- 升级前冻结在已移除机制点的多人时间提案会失效；重新发起的提案按新时间线确认和推进。新提案不会因为快照尚未写回而反复失效。
- 机制截止时刻可以超过短篇终点：抵达终点仍未到期的状态/疗程继续保留，剧情不会为了等待机制而延长。

本轮验收：

| 检查 | 结果 |
|---|---|
| framework全量（含原剧情TimeTask规则分派） | **731 passed** |
| SQLite四阶段持久化及全部时间提案回归 | **30 passed** |
| PostgreSQL四阶段持久化及新旧多人提案 | **11 passed** |
| SDK→真实后端→WS→重连 | 临时疯狂、不定性疯狂各 **1 passed** |
| 静态与契约检查 | 后端Ruff/格式/ty、修改的framework文件Ruff/格式、E2E TypeScript均通过；公开Schema无漂移 |
| 现有开发库快照读取 | **17份可读**，迁移版本保持 `l5m6n7o8p9q0` |

后端全量尝试不能记为通过：中断时为755 passed、23 skipped、2 failed。其一是新增验收错误预设开场为18:00，实际夹具为06:00；改为验证一次正常12小时跳转跨越两项期限后，上述SQLite 30项和PostgreSQL 11项均通过。另一项 `test_pending_keeper_resubmit_does_not_fail_completed_queue_claim` 在WebSocket断开提交 `Player.connected` 时遇到SQLite锁，并阻塞清理，已中断本轮全量。该问题也存在于本次修改前的 [PR后端CI](https://github.com/1024XEngineer/TRPG-master/actions/runs/34748844683)，本提交不宣称修复该WebSocket问题。

修订期间还发现原PR临时疯狂E2E缺少两个可选字段断言，已补充并通过TypeScript类型检查。同步前已在私有临时目录保存开发库备份 `before-mechanical-deadlines-20260913110016.db`；`.env`、原计划、用户IDE文件和PPT保留校验值。专用PostgreSQL测试实例已关闭。


## PR #532 CI 修复：完整 E2E 入口与场景隔离

失败日志来自 E2E CI `34753449018`：完整入口共51项，43 passed、5 failed、3 skipped。
本地完整运行复现了相同的5个失败用例。后端、前端、SDK、迁移和契约漂移检查在该提交的CI均通过。

- 《追书人》已经发布3.0.13，目录E2E仍断言3.0.12，现同步为本PR发布版本。
- 四条SAN穿行此前各自验证时显式传入了不同的有限骰列，默认CI入口没有这些参数，仍使用固定骰值1，因此预期81的失败检定没有发生。统一入口现自动识别场景配置；全量和单文件选择使用同一份骰列。
- 有限骰列属于后端应用进程，不能供并行文件争用。普通场景先运行，四条SAN场景分别在新后端进程和全新测试数据库中依次运行；等待旧进程退出后才清理测试库和启动下一场景。任何场景失败均保留非零退出码，未跳过原有断言。
- 两个发布测试模组变体的夹具缺少SQLite锁等待，完整并行执行时出现database is locked。现设置busy_timeout，并以BEGIN IMMEDIATE事务一起发布版本和绑定房间。
- 四条SAN场景捕获行动Promise的失败，并在正常路径断言行动完成后再断线，避免断言失败后的清理又产生未处理拒绝。

本地使用与CI一致的Node 22.23.2完成TypeScript检查和默认完整入口：**48 passed、0 failed、3 skipped**，包括四条SAN穿行各1 passed。3项跳过仍是既有真实模型条件／已知阻塞场景，未新增跳过。另验证单文件命令不传骰列时1 passed；显式将D100改为1时，原81断言按预期失败、命令退出1，且没有异步清理引发的未处理拒绝。

预览部署 `34753449000` 的镜像构建成功，失败发生在预览服务器连接ghcr.io时的TLS握手超时，属于与上述E2E失败独立的网络错误。推送修复后继续检查新一轮部署结果。
