/** SDK → real backend cumulative insanity. Run with E2E_DICE_BY_SIDES='{"100":[81,81,81,81],"6":[4,4,3,1],"10":[3,2]}' */
...
function seedCemeteryStench(roomId: string): void {
  ...
  try {
    ...
    const state = JSON.parse(row.state_json) as {
      ...
      actors: Record<string, { resources: { san: number }; conditions: string[]; sanity: { window: { baseline_san: number }; losses: unknown[] } }>
    }
    state.scene_id = 'cemetery'
    for (const actor of Object.values(state.actors)) {
      assert.equal(actor.sanity.losses.length, 0, '设定开局夹具时尚未发生任何 SAN 损失')
      actor.resources.san = 60
      actor.sanity.window.baseline_san = 60
    }
...
/** Publish a test-only variant with four sequential rule-owned exposures. */
function seedRepeatedChecks(roomId: string): void {
  ...
  try {
    ...
    content.version = 'test-indefinite-e2e'
    const rule = content.rules.find((r: { id: string }) => r.id === 'ghoul_crowd_sanity')
    const check = rule.execution.steps.find((s: { kind: string }) => s.kind === 'check')
    content.sanity_sources = [0, 1, 2, 3].map(i => ({ id: `test.horror.${i}`, habit_cap: null }))
    const checks = [0, 1, 2, 3].map(i => ({ ...structuredClone(check),
      check: { ...structuredClone(check.check), parameters: { success_loss: '0', failure_loss: '1d6', sanity_source: `test.horror.${i}` } }, id: i === 0 ? 'crowd_san' : `crowd_san_${i}`,
      result_routes: Object.fromEntries(Object.keys(check.result_routes).map(k => [k, i < 3 ? `crowd_san_${i+1}` : 'finish'])) }))
    rule.execution.steps = [rule.execution.steps[0], ...checks, { id: 'finish', kind: 'finish' }]
...
function probeWorld(roomId: string): WorldProbe {
  ...
  try {
    ...
    return {
      ...
      conditions: Object.values(state.actors)[0].conditions,
...
test(
  'SAN 60 → 56 → 52 → 49 → 48 causes indefinite insanity, without INT, across reconnect',
  ...
  async () => {
    ...
    try {
      ...
      seedRepeatedChecks(room.roomId)
      ...
      assert.equal(midway.san, [60, 56, 52, 49][i])
      assert.equal(midway.facts, i)
      ...
      const settled = waitForEvent(room.host.sdk, (event) => i < 3 ? event.type === 'adjudication.pending' && event.payload.status === 'awaiting_skill_choice' : event.type === 'narration.push')
      ...
      assert.equal(probeWorld(room.roomId).san, [56, 52, 49, 48][i])
      if (i < 3) pendingEvent = next as PendingAdjudicationEvent
      ...
      assert.equal(final.san, 48)
      assert.equal(final.facts, 4)
      assert.deepEqual(new Set(final.conditions), new Set(['indefinite_insanity', 'madness_bout']))
      ...
      await rebound
      await restored
      assert.equal(probeWorld(room.roomId).san, 48)
      const view = room.host.sdk.roomSocket.getPlayerView()
      assert.ok(view)
      assert.equal(view.self_actor.resources.find(resource => resource.id === 'san')?.value, 48)
      const indefinite = view.self_actor.condition_details?.find(c => c.id === 'indefinite_insanity')
      assert.ok(indefinite)
      assert.equal(indefinite.remaining_hours, null)
      assert.equal(indefinite.recovery_status, 'care_required')
      assert.equal(view.self_actor.condition_details?.find(c => c.id === 'madness_bout')?.remaining_hours, 2)
      assert.equal(view.self_actor.condition_details?.some(c => c.id === 'temporary_insanity'), false)
    } finally {
      room.host.sdk.roomSocket.disconnect()
    }
  },
