...
/** Publish a test-only variant with three sequential rule-owned exposures. */
function seedRepeatedChecks(roomId: string): void {
  ...
  try {
    ...
    content.version = 'test-habituation-e2e'
    ...
    const checks = [0, 1, 2].map(i => ({ ...structuredClone(check), id: i === 0 ? 'crowd_san' : `crowd_san_${i}`,
      result_routes: Object.fromEntries(Object.keys(check.result_routes).map(k => [k, i < 2 ? `crowd_san_${i+1}` : 'finish'])) }))
...
test(
  'SAN 60 → 56 → 54 → 54 with a shared cap and reconnect',
  ...
  async () => {
    ...
    try {
      ...
      assert.equal(midway.san, [60, 56, 54][i])
      ...
      const settled = waitForEvent(room.host.sdk, (event) => i < 2 ? event.type === 'adjudication.pending' && event.payload.status === 'awaiting_skill_choice' : event.type === 'narration.push')
      ...
      assert.equal(probeWorld(room.roomId).san, [56, 54, 54][i])
      if (i < 2) pendingEvent = next as PendingAdjudicationEvent
      ...
      await rebound
      await restored
      assert.equal(probeWorld(room.roomId).san, 54)
      const view = room.host.sdk.roomSocket.getPlayerView()
      assert.ok(view)
      assert.equal(view.self_actor.resources.find(resource => resource.id === 'san')?.value, 54)
    } finally {
      room.host.sdk.roomSocket.disconnect()
    }
  },
