/** SDK → real backend temporary insanity. Run with E2E_DICE_BY_SIDES='{"100":[81,21],"6":[5],"10":[8,3,2]}' */
...
test(
  'SAN loss → INT → temporary insanity → authored asylum rule → reconnect',
  ...
  async () => {
    ...
    try {
      ...
      const settled = waitForEvent(room.host.sdk, (event) => event.type === 'adjudication.pending' && event.payload.status === 'awaiting_skill_choice')
      ...
      const intPending = await settled as PendingAdjudicationEvent
      assert.equal(probeWorld(room.roomId).san, 55)
      const intDecision = intPending.payload.pendingDecision
      assert.ok(intDecision)
      assert.equal(intDecision.options[0].skill_id, 'INT')
      const intRollPromise = waitForEvent(room.host.sdk, event => event.type === 'adjudication.pending' && event.payload.status === 'awaiting_post_roll_decision')
      room.host.sdk.roomSocket.selectAdjudication(room.hostPlayerId, {
        clientActionId: actionId, requestId: `${actionId}-int-select`, sourceRevision: intPending.payload.sourceRevision,
        decisionId: intDecision.decision_id, decisionVersion: intDecision.decision_version, candidateId: intDecision.options[0].candidate_id,
      })
      const intRolled = await intRollPromise as PendingAdjudicationEvent
      ...
      room.host.sdk.roomSocket.decidePostRoll(room.hostPlayerId, {
        clientActionId: actionId, requestId: `${actionId}-int-accept`, sourceRevision: intRolled.payload.sourceRevision,
        checkId: intRun.check_id, checkVersion: intRun.version, optionId: 'accept-current',
      })
      await narration
      ...
      assert.ok(view)
      assert.equal(view.self_actor.resources.find(resource => resource.id === 'san')?.value, 55)
      ...
      const database = new DatabaseSync(DB_FILE, { readOnly: true })
      try {
        const row = database.prepare('SELECT state_json FROM game_sessions WHERE room_id=?').get(room.roomId.replaceAll('-', '')) as { state_json: string }
        assert.equal(JSON.parse(row.state_json).entities.case_tracker.sent_to_asylum, true)
