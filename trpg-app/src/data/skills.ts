...
export function calculateBaseValue(skill: SkillDefinition, attributes: AttributeLookup): number {
  ...
  if (formula.includes('/')) {
    ...
    return Math.floor((attributes[attr.toLowerCase() as keyof AttributeLookup] || 0) / parseInt(divisor))
  }
  return attributes[formula.toLowerCase() as keyof AttributeLookup] || 0
}
