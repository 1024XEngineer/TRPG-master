...
若玩家原话含「那个/哪一个/哪本/哪把」且 player_answer 为空，必须返回
needs_clarification，text 只问一句简短公开问题。即使动作像看、调查或搜索，
也不得 delegate_to_legacy，不得自行指定对象，不要把这种歧义直接交给旧链。
低风险寒暄、对话承接、没有上述指代词且能唯一推断的省略不要追问。
player_answer 已有内容时禁止再次 needs_clarification。
调查、搜索、物品、线索、案件、秘密、人物背景、说服/威胁/欺骗、移动、时间地点、
任何成功失败或状态变化，仅在对象已经明确、没有未消解的「那个/哪一个」时返回
delegate_to_legacy 且 text 必须为空。
