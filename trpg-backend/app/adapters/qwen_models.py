...
class QwenChatCompletionsJsonClient(StructuredJsonClient):
    ...
    async def generate(
        ...
        *,
        ...
        output_text = _chat_completion_output_text(response.json())
...
def _chat_completion_output_text(payload: object) -> str:
    if not isinstance(payload, dict):
        raise ValueError("Qwen Chat Completions payload must be an object")
    choices = payload.get("choices")
    if not isinstance(choices, list) or not choices:
        raise ValueError("Qwen Chat Completions payload has no choices")
    first = choices[0]
    if not isinstance(first, dict):
        raise ValueError("Qwen Chat Completions choice must be an object")
    message = first.get("message")
    if not isinstance(message, dict):
        raise ValueError("Qwen Chat Completions choice has no message")
    content = message.get("content")
    if not isinstance(content, str) or not content.strip():
        raise ValueError("Qwen Chat Completions message has no text content")
    return content


__all__ = ["QwenChatCompletionsJsonClient"]
