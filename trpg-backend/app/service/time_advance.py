...
def _resolved_payload(record: TimeAdvanceProposalRecord) -> TimeAdvanceResolvedPayload:
    ...
    return TimeAdvanceResolvedPayload(
        ...
        target_hour_of_day=record.target_hour_of_day,
        committed_revision=(
            str(record.committed_revision)
            if record.committed_revision is not None
            else None
        ),
    )
...
async def create_from_adjudication(
    ...
    if any(
        isinstance(effect, AdvanceWorldTimeEffect)
        for effect in adjudication.failure_effects
    ):
    ...
    player_by_actor = {
        actor_id: actor.player_id for actor_id, actor in state.actors.items()
    }
    ...
    try:
        ...
        # 多实例可能同时观察到“没有 active 提案”。数据库用 room+revision
        # 决定唯一赢家；同一动作复用赢家，其他动作得到明确冲突。
        await db.rollback()
        winner = await db.scalar(
            select(TimeAdvanceProposalRecord).where(
                TimeAdvanceProposalRecord.room_id == request.room_id,
                TimeAdvanceProposalRecord.source_revision == source_revision,
            )
        )
...
async def _respond_locked(
    ...
    *,
    ...
    # 重复点击必须幂等：已记录的同意不再受旧 proposalVersion 影响。
    already_accepted = accept and player_id in record.accepted_player_ids
    if not already_accepted and proposal_version != record.proposal_version:
        raise TimeAdvanceError("时间提案已更新，请按最新状态重试")
    ...
    if session is None or session.state_version != record.source_revision:
        record.status = "stale"
        record.proposal_version += 1
        await db.commit()
        return _resolved_payload(record), record.player_id, record.parent_action_id
    ...
    if not already_accepted:
        record.accepted_player_ids = sorted(
            {*record.accepted_player_ids, player_id}
        )
        record.proposal_version += 1
