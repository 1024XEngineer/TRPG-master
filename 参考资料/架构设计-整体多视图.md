...
| 账号体系 | ✅ | 2026-07-10 定：MVP 起就做（不后置），自定义账号+密码；**2026-07-11 起由「可选层」改为强制前提**（ADR-16 推翻 ADR-15 的「不强制访客注册」结论）——登录是玩游戏的硬性前提，不再只是「换设备/重装找回」的可选增强，见 §4.4.8/ADR-15/ADR-16 |
...
World {                              // 系统层，只读，随游戏类型新增（COC 首个实现）
  ...
  sanityMechanic: SanityMechanicDef  // SAN 损失触发规则、临时/不定性疯狂阈值；🆕 2026-07-11 补 severityTable:{mild/moderate/severe/extreme: SanLossSpec}，供 AI 临场触发 SAN 用，见 [[AI编排架构详细设计]] §2.2
...
interface Checkpoint {                // 检定点
  ...
  difficulty?: 'regular' | 'hard' | 'extreme';   // ★ 允许为空：难度由玩家自然语言质量决定时，由 LLM 上游求值出枚举写入状态槽，此处留空（软判据与硬求值分离，详见 [[AI编排架构详细设计]] §1.3）
...
| 表达式变量引用 | `Rule.when`/`WinCondition.expr` 里出现的每个变量，必须在 `World.variables`（4.3.6.2）表内 | 软判据与硬求值分离（详见 [[AI编排架构详细设计]] §1.3）——引用表外变量（如误写 `self.mood`）直接判失败，打回重新生成；`spawn` 表达式引用的 `party.size` 等同理（§6.7 数量动态缩放） |
...
| **敏感字段类型级隔离** | `entity.secrets`（`kind` 为 npc/monster 时的底牌）只进 `GameState`（GodView），`ViewProjector`/`Narrator` 的类型签名**结构上拿不到它**——与 §六.6.2「不泄底」是同一机制，模组导入不能绕过。🔴 2026-07-13：骨架代码首次实现 `ViewProjector` 时发现 `PlayerView.visibleClues` 此前直接复用了带 `secrets` 的 `Entity` 类型，没有真正做到"结构上拿不到"，已改用窄类型修复，见 §4.5 `PlayerView` 定义处的补记 |
...
无循环、无递归、无函数定义——约二三百行可实现，天然安全（不会死循环、不会越权）。**关键约束**：`when`/`expr` 里不能放语义判据（如「玩家表述得很精彩」）——语义判据必须由 LLM 在**上游**求值，输出枚举写入状态槽，规则只读枚举（软判据与硬求值分离，详见 [[AI编排架构详细设计]] §1.3）。
...
erDiagram
    ...
    ROOMS {
        ...
        string host_user_id FK "非空，2026-07-11起，见4.4.8"
    }
    PLAYERS {
        string id PK
        string room_id FK
        string character_id FK "denormalized，见下方说明"
        string user_id FK "非空（is_ai=true例外），2026-07-11起，见4.4.8"
        bool ready
        bool connected
    }
    CHARACTERS {
...
- **`USERS` 的三条关系线，2026-07-11 起只有 `owner_user_id` 仍是可选（0 或 1）**：图中 `||--o{` 记法本图统一不区分必填/可选（同 `based_on_pregen_id` 的处理）。`module_packs.owner_user_id`（模组导入归属，内置模组无主）继续可空；但 `host_user_id`/`user_id` 已改为**非空**（登录从可选层升级为强制前提，`players.is_ai=true` 的 AI 队友行例外仍可空）——真人玩家必然产生这两条关系，见 §4.4.8/ADR-16。
...
| 表 | 关键字段 | 说明 |
|---|---|---|
| `rooms` | `id`(PK) / `room_code`(unique) / `host_player_id` / **`host_user_id`**(FK→users.id，**非空**，2026-07-11起) ★ / `module_pack_id`(FK) / `phase`(枚举，对应 §5.2.4 状态机) / `entity_states`(JSONB，`Record<EntityId, Record<string,Primitive>>`) / `rolling_summary`(text，§6.5) / `created_at` / `updated_at` | **GameState 字段直接并入 `rooms` 表**（1:1）。★ **本次移除 `current_scene_id`**（迁移至 `characters.location`，见下，支持分头行动）与 **`discovered_clue_ids`**（折进 `entity_states`：每个 `kind=clue` 的 entity 在状态里带一个 `discovered: boolean` 键，由授予线索的 Op 置真——避免同一事实存两份，与 §4.3.1 删除 `reachable_by` 是同一类简化）。`host_user_id`（2026-07-10 新增，2026-07-11 起改为非空）：房主账号，登录强制前提下必然存在，用于「查我创建过的房间历史」，见 §4.4.8 |
| `players` | `id`(PK) / `room_id`(FK) / `nickname` / `character_id`(FK，nullable 直到车卡完成) / **`user_id`**(FK→users.id，nullable) ★ / `ready`(bool) / `unknown_streak`(int，默认 0，§6.6) / `connected`(bool) / `last_event_id`(§5.2.5 重连锚点) / `joined_at` / `left_at`(nullable) / `reconnect_token`(nullable，2026-07-12 补) | 索引 `(room_id)`、`(room_id, connected)`。`user_id`（2026-07-10 新增，nullable）：未登录的游客为空，登录用户关联对应 `users.id`，见 §4.4.8。**2026-07-12 补充** `joined_at`/`left_at`/`reconnect_token`（见 [[前端demo数据表设计]]）：断线/中途退出这一行不删，只把 `connected` 置 false 并回填 `left_at`；`reconnect_token` 是 `session.bound`（§5.2.2）签发凭证的服务端持久化落点，此前只在协议层描述、没有对应存储列——`room.rejoin` 校验的正是这一列。**命名对齐**：[[前端demo数据表设计]] 曾用 `is_connected` 指代同一个字段，本节 `connected` 是先建立的名字，两文档统一以 `connected` 为准，该文档已同步改名 |
...
| `users` | `id`(PK) / `account`(unique，用户自定义登录名) / `password_hash` / `nickname`(默认昵称，可被 `players.nickname` 逐局覆盖) / `created_at` / `updated_at` | ★2026-07-10 由占位转正（ADR-15），**2026-07-11 起登录改为强制前提（ADR-16）**：创建/加入房间、车卡均需已登录，不再是「游客无需注册也能玩」；浏览游戏/模组目录不受影响，仍不需要登录，机制见 §4.4.8 |
...
房间码是「知道去哪个房间」的门槛，此前**长度/字符集/是否防猜测均未定义**——码太短/太规律，等于「门」形同虚设。（🔴 2026-07-11 更正：此前这里写"房间码是访客加入房间的准入门槛、账号可选不影响这条设计"——账号已从可选层升级为强制前提（ADR-16），房间码不再是唯一门槛，还需先登录；这条规格本身（熵值/过期/限流）不受影响，房间码依然需要防枚举）规格：
...
#### 4.4.8 🟢 账号体系与跨设备找回（2026-07-10 新增，`users` 由占位转正；2026-07-11 登录由可选升级为强制，见 ADR-16）

> 🔴 **方向调整，如实记录**：本节 2026-07-10 定稿时，账号是「可选层」——游客可以完全不登录、只靠房间码+`reconnectToken` 玩一局。**API 详细设计阶段（2026-07-11）推翻了这条**：产品诉求明确「每个玩家都要能复盘、掉线后能重新进来」，而 `reconnectToken` 只解决同设备内的会话级重连，解决不了跨设备/隔一段时间后的找回——要满足这个诉求，每个真人玩家必须有服务端持久身份，登录因此从「可选增强」变成「玩游戏的硬性前提」。详见 ADR-16、[[2026-07-11 账号强制登录-推翻不强制访客注册]]。下文按新结论更新，不再是「可选层」表述。

`users` 表此前只是占位（§4.2「MVP 先房间码轻量身份」）。本次补齐字段，触发原因是产品要求支持「关闭 App、换设备或重装后，找回同一局游戏/同一个角色」——这与已有的 `reconnectToken` 机制（§5.2.5）解决的不是同一个问题，两者是**叠加关系，不是替代**（账号解决跨设备/跨时间找回，`reconnectToken` 解决同一局进行中的快速重连；2026-07-11 起，账号从「可选叠加层」变成每个真人玩家的**前提**，`reconnectToken` 仍然叠加在账号之上处理会话级重连，两者分工不变）：
...
**两处外键**（2026-07-11 起**非空**，此前是「均可为空」——`players.is_ai=true` 的 AI 队友行例外，继续可空，因为 AI 没有真实账号）：
- `players.user_id` → `users.id`：这局游戏这个玩家关联的账号；真人玩家必须关联（登录强制前提），AI 队友（`is_ai=true`）例外仍可空。
- `rooms.host_user_id` → `users.id`：房主账号，支持查「我创建过的房间历史」（`GET /users/me/rooms`，见 API 对齐规范）；`module_packs.owner_user_id`（早先已预留的字段）现在终于有了对应的表。
...
**App 的实际门槛（2026-07-11 起）**：打开 App 可以不登录浏览游戏/规则系统/模组目录（相当于看一遍 demo）；但创建房间、加入房间、车卡这些涉及具体玩家身份的动作，都要求已登录——不再是「扫码加入房间的临时玩家可以完全不碰账号体系」。注册与加入房间是两个独立、完整的步骤，故意不合并：账号密码必须玩家本人主动设置，系统代填的话换设备就登不回来，账号形同虚设。

**MVP 范围边界（明确不做）**：不做密码找回/重置流程（忘记密码是已知限制，不阻塞架构，落地期再补）；不做账号间社交关系（好友/关注）。`password_hash` 哈希算法（如 bcrypt/argon2）留到实现期选，不影响此处设计。

#### 4.4.9 🟡 数据表设计的其余新表（2026-07-11 补充索引，完整字段见独立文档）

> **文档策略（2026-07-11 用户明确要求）**：本节只列表名+一句话+权威出处，**不复制字段级细节**——避免 master 内容过分冗余。完整字段/理由/17个开放问题的拍板过程，一律以 [[前端demo数据表设计]] 为准，master 不重复维护，两者不一致时以数据表设计文档为准（这条与"数据表设计/master/API对齐规范三份文档并存、不整体合并"的既定策略一致，见 [[aidm-architecture-doc-strategy]]）。

除 §4.3/§4.4.1~4.4.8 已有的内容层/GameState/账号表之外，[[前端demo数据表设计]] 还定义了以下表，master 之前完全没有索引到：

| 表 | 一句话 |
|---|---|
| `games` / `game_systems` | 游戏大类 + 规则系统目录（跑团/血染钟楼/…；COC7/DND5e/…），支撑 §5.1 `GET /games`/`GET /games/{gameId}/systems` |
| `user_sessions` | 真实服务端登录会话（非无状态 JWT），支撑 §4.4.8 账号体系的登出/失效 |
| `room_summaries` / `room_sessions` | 复盘摘要（1:1）+ 上线场次记录，支撑 §5.1 `GET /rooms/{roomId}/summary` |
| `module_import_jobs` | 模组导入这个异步任务的状态追踪（`queued`/`parsing`/`validating`/`failed`/`succeeded`，含 `retry_count`），支撑 §5.1 `POST/GET /modules/import(/{importJobId})` 的异步化（见上方 §5.0 更正） |
| `worlds.occupationCatalog`/`skillCatalog` 扩展字段 | 职业/技能目录（含技能槽位类型、建卡可分配性标记等），支撑 §5.1 `GET /systems/{systemId}/ruleset` 建卡规则数据 |
| `worlds.ageModifierTable`/`pointBuyConfig`/`weaponCatalog` | 建卡年龄修正表/点数购买配置/武器目录（`weaponCatalog` 后置，跟战斗规则引擎一起做） |

`characters`/`rooms`/`players` 这几张 master 已有的表，数据表设计文档做了大量字段级细化（建卡 bio 字段、`equipment` 结构、`attribute_gen_method` 等）——这些是对已有表的**深化**，不是新表，细节同样以数据表设计文档为准。

### 4.5 运行时/模块接口契约（原 MS1 §四，已并入并对齐新 schema）

> **代码即架构 · 接口为真、实现留空**：接口类型同时把 §4.3 的数据模型在代码层面固定下来。**TS 记法**，落地时按 ADR-9 转 Python/Pydantic（字段名同构，`camelCase`→`snake_case`）。相对 MS1 原版做了字段对齐：`Module`→`ModulePack`（内容已细化进 §4.3）、`PlayerState` 补 `unknownStreak`（§6.6）；**2026-07-10 再次对齐**：`GameState` 补 `entityStates`、移除 `currentSceneId`/`discoveredClueIds`（原因见 §4.4.2/ADR-12）、`PlayerState` 补 `location`、`Event.payload` 从 `unknown` 展开为判别式联合类型。**2026-07-11 AI 编排详细设计第一轮再次对齐**：`Intent` 目标字段改用 `Ref` 判别结构、新增 `SceneActionMenu`/`skillCheck`，`ActionResult` 补 `resolutionKind`/`appliedOutcome`，原 `CheckOutcome`（骰子结果）改名 `CheckRollResult` 以消除与模组内容同名类型的撞名，详见 [[AI编排架构详细设计]] §一。

```typescript
// ===== 运行时状态（GameState = God View，仅 Rules 可写；对应 §4.4.2 rooms 表）=====
interface GameState {
  roomId: string;                  // ★ 2026-07-13 补记：此前遗漏，与 Event/EventLog 同一类问题——save(state) 若不带 roomId，真实实现无法知道该写回哪个房间的 rows（load(roomId) 有房间参数，但对称的 save(state) 全靠 state 自带才行）；骨架阶段 load→mutate→save 都在同一进程内用同一个对象，容易被忽略，真正接真实数据库才会暴露
  ...
  ~~events: Event[];~~              // ★ 2026-07-13 补记：骨架代码首次真实实现 GameState 时发现这一行是死语——§六.7 prompt 分层表自己就写明"近期原文"来自 EventLog 单独查询（层⑤：EventLog 近期条目），不是从 GameState 里带。真把全量历史事件塞进每回合都要 load 一次的 GameState，既没被下游用到，也是明显的内存/查询浪费。代码里的 GameState（state/models.py）从一开始就没有这个字段，是代码对、这里文档滞后，此处删除，不是新的设计决策
}
...
interface PlayerView {
  ...
  visibleClues: PlayerVisibleClue[]; // 🔴 2026-07-13 补记：原写 Entity[]，但 Entity 类型带 secrets 字段——喂给 LLM 的类型签名里结构上能拿到 secrets，跟 §4.3.4"敏感字段类型级隔离"自相矛盾（那条明确要求 ViewProjector/Narrator 的类型签名结构上拿不到 secrets）。改成只含 { id, name, content } 的窄类型，真正做到类型层面切断。筛选条件不变：kind='clue' 且 entityStates[id].discovered=true
  visibleSan: number;
  // 注意：绝不含未触发的底牌 / 暗骰结果 / 其他玩家私密信息 / Entity.secrets（§4.3.1）
}

interface SceneActionMenu {          // 🆕 2026-07-11，仅供 IntentParser 内部解析用，不进 view.private / 不下发客户端，理由见 [[AI编排架构详细设计]] §1.2
  entities: { id: string; name: string }[];    // 当前 Scene.contents 里 entity_present/clue_access 关联的 Entity，仅 id+name
  exits: { sceneId: string; title: string }[]; // Scene.exits 对应的可去地点，可下发客户端（跟 entities 不同，见同上）
}

// ===== 意图与裁决 =====
type Ref = { matched: true; id: string } | { matched: false; text: string };   // 🆕 软判据(LLM匹配)→硬求值(Rules只读)的标准结构，见 [[AI编排架构详细设计]] §1.3
type Intent =
  | { kind: 'investigate'; target: Ref }         // 🔧 原 target: string
  | { kind: 'move'; toScene: Ref }                // 🔧 原 toSceneId: string
  | { kind: 'talk'; npc: Ref; utterance: string } // 🔧 原 npcId: string
  | { kind: 'skillCheck'; skill: string }         // 🆕 路径B：玩家主动选技能摇骰，不经过内容匹配
  | { kind: 'ask'; question: string }          // 提问/答疑(脱本)
  | { kind: 'unknown'; raw: string };           // 触发 §6.6 脱本导回状态机

interface ActionResult {
  ok: boolean;
  resolutionKind: 'checkpoint' | 'direct' | 'improvised' | 'blocked' | 'unrecognized';  // 🆕 五路分支判别，见 [[AI编排架构详细设计]] §1.4
  roll?: CheckRollResult;         // 🔧 原 check: CheckOutcome，改名+改类型；checkpoint 与 improvised(Tier2) 都可能有值
  appliedOutcome?: CheckOutcome;  // 🆕 仅 resolutionKind==='checkpoint' 时有值——命中的 onSuccess/onFail 效果套餐本体，结构性保证临场判定不执行预设效果
...
interface CheckRollResult { skill: string; roll: number; target: number; success: boolean; hidden: boolean }  // 🔧 原名 CheckOutcome，2026-07-11 改名——与 §4.3.1 模组内容的 CheckOutcome（narrationHint/grantsEntityIds/sanLoss/sceneTransition）撞名，两者含义完全不同

// ===== EventLog 持久化事件：payload 判别式联合类型（原 unknown，2026-07-10 展开；visibility 同日由两档扩为三档）=====
interface Event { id: string; roomId: string; ts: number; playerId?: string; visibility: 'public' | 'scene' | 'private'; payload: EventPayload }  // 对应 §4.4.3 events 表，type 由 payload.type 判别。🔴 2026-07-13 补记：此前这里漏了 roomId——§4.4.3 物理表一直有 room_id 列且建了 (room_id, id) 索引，但这条逻辑接口没把它暴露出来，导致 EventLog.list() 根本没法表达"查某个房间的事件"，是骨架代码首次接入真实持久化时发现的真实缺口（此前只有桩实现，没人真正调用过）。修法：Event 加 roomId 字段，EventLog.list() 加 roomId 必填参数（见下）
type EventPayload =
  ...
  | { type: 'san.check.result'; playerId: string; sanLossFinal: number; luckSpent?: number }   // 🆕 2026-07-12 补：此前 check.result（技能检定）有落库变体，san.check.result（理智检定）没有，会导致 GET /replay 漏掉理智检定历史，属遗漏而非刻意不做
...
interface IntentParser  { parseIntent(utterance: string, view: PlayerView, menu: SceneActionMenu): Promise<Intent> }  // 🔧 加 menu 参数；LLM，按 role='intent' 查 §6.3 ModelRoleConfig 选实现
interface CheckResolver { roll(skill: string, target: number, hidden: boolean): CheckRollResult }           // 🔧 返回类型改名，见上
...
interface SanJudge      { judge(sceneText: string): Promise<{ trigger: false } | { trigger: true; severity: 'mild'|'moderate'|'severe'|'extreme' }> }  // 🆕 LLM，见 [[AI编排架构详细设计]] §2.2；仅在没有预设 SanTrigger 命中时由 RulesEngine 调用，severity 查 World.sanityMechanic.severityTable 换算成确定性 SanLossSpec，不产出 flat/direct/max_reduce/gain/capped 这类更强的效果
interface ViewProjector { project(state: GameState, forWhom: string): PlayerView; projectActionMenu(state: GameState, forWhom: string): SceneActionMenu }  // 🔧 加 projectActionMenu；权限唯一出口（通信铁律一）
type Persona = 'narrator' | 'npc' | 'qa';                                                                    // 对应 §6.3 ModelRole（qa=答疑）
interface Narrator      { narrate(persona: Persona, view: PlayerView, result: ActionResult): AsyncIterable<string> }  // LLM，流式（对应 5.2.2 narration.push streaming=true，🔧 2026-07-11 补：迭代完成即对应 narration.push.done=true，见 [[AI编排架构详细设计]] §三）
interface QAResponder   { answer(question: string, view: PlayerView): AsyncIterable<string> }  // 🆕 2026-07-11，只读、不产生 ActionResult，见 [[AI编排架构详细设计]] §四
interface RoomSummary {  // 🆕 2026-07-12 补：此前只被 SummaryGenerator 引用、未给出定义，字段取自 §4.4.9 room_summaries 表 + API 对齐规范 §2.7 响应形状
  roomId: string;
  endingType: string;                 // 命中的 WinCondition.id
  summaryText: string;                // 唯一需要 LLM 生成的部分
  keyFindings: string[];              // 查 entityStates 里 kind='clue' 且 discovered=true
  stats: { sanChanges: Record<string, number>; characterFates: Record<string, string>; durationMinutes: number };
  generationMethod: 'llm' | 'fallback_template';   // §5 SummaryGenerator "绝不失败"降级标记
  generatedAt: number;
}
interface SummaryGenerator { generate(state: GameState, hitWinCondition: WinCondition): Promise<RoomSummary> }  // 🆕 2026-07-11，输入是 GameState 全貌(GodView)不是 PlayerView——复盘阶段不受"不泄底"约束，呼应 §5.1 `GET /replay` 全事件可见的既有边界；见 [[AI编排架构详细设计]] §五
interface EventLog      { append(e: Event): void; list(roomId: string, sinceId?: string, visibility?: 'public' | 'scene' | 'private'): Event[] } // 🔴 2026-07-13 补记：list() 加了必填 roomId 参数（原签名没有，见上方 Event 的补记）——append 不用改，room_id 已经在 event.roomId 里带过去了。list 参数支持 §5.2.5 断线重连查询（重连只用 'public'）；实现上是 GameStateRepo 管辖范围内的一张表，接口单独列出是因为调用方（EventLog 消费者）不同
...
**模组导入(§4.3.3)天然是 REST**：客户端一次性上传原始材料 + 资产，🔴 **2026-07-11 更正**：此前这里写"服务端跑六步校验，同步返回结果"——这个假设成立的前提是上传的已经是一份规整的 `ModulePack` JSON，但真实场景是用户上传原始材料（PDF 为主，地图多半嵌在 PDF 内部），六步校验之前多了一道**耗时不定、可能失败**的「LLM/Agent 解析生成 ModulePack 候选」步骤，不能再假设同步返回。REST 本身没变（仍是一次性提交、非长连接的资源操作），只是从「一次请求拿到最终结果」改成**提交任务(202)+轮询状态**两段式，详见 [[API接口对齐规范]] §2.4、[[2026-07-11 模组导入接口-同步改异步与失败兜底]]。

### 5.1 REST 端点一览（资源管理）

> 🔴 **2026-07-11 API 详细设计阶段大幅扩充**，本表补齐当时缺失的端点；完整请求/响应示例见 [[API接口对齐规范]]，本表只列路径与一句话职责。

| 方法 | 路径 | 说明 |
|---|---|---|
| `POST` | `/auth/register` | ★新增：注册（账号+密码+昵称均玩家自己填），成功直接返回可用 token |
| `POST` | `/auth/login` | ★新增：登录 |
| `POST` | `/auth/logout` | ★新增：登出，撤销当前 `user_sessions` |
| `GET` | `/auth/me` | ★新增：查当前登录账号信息 |
| `GET` | `/games` | ★新增：游戏大类列表，不需要登录 |
| `GET` | `/games/{gameId}/systems` | ★新增：该大类下的规则系统列表，不需要登录 |
| `POST` | `/rooms` | 创建房间，返回 `roomId` + `roomCode` + `reconnectToken`（🔒 需登录，房主创建即加入，见 §5.2.5）；不带 `gameId`/`systemId`——选游戏/选系统是纯前端导航，不落后端 |
| `GET` | `/rooms/{roomCode}` | 查询房间基本信息（供加入前预览，不强制登录） |
| `GET` | `/modules` | 模组目录（内置 + 该账号已导入），字段取自 `ModuleMeta`（§4.3.1），支持 `?systemId=` 筛选 |
| `POST` | `/modules/import` | 上传模组材料，跑「LLM解析→六步校验」，返回 `202`+`importJobId`（🔒 需登录，2026-07-11 改为异步，见上） |
| `GET` | `/modules/import/{importJobId}` | ★新增：轮询导入任务状态（`queued`/`parsing`/`validating`/`failed`/`succeeded`），仅上传者本人可查 |
| `GET` | `/modules/{moduleId}` | 模组详情（供选模组屏展示） |
| `POST` | `/rooms/{roomId}/module` | 房主确定选用的模组，`{moduleId, attributeGenMethod?}`（🔒 需登录仅房主），幂等条件更新（`WHERE scenario_id IS NULL`）防竞态，已设置过返回 `409`，触发 `Lobby→ModuleSelected`（见 §5.2.4）。`attributeGenMethod`（`roll`/`point_buy`，2026-07-11 新增）跟选模组同一时间段一并提交，不单独开端点 |
| `GET` | `/systems/{systemId}/ruleset` | ★新增：建卡所需规则数据一次性拉取（属性/技能/职业目录/年龄表/点数购买配置/衍生属性算法），🔒 需登录 |
| `POST` | `/rooms/{roomId}/characters` | 车卡：`{basedOnPregenId}` 一步到位 `complete`，或空创建拿到 `draft` 骨架走断点续建（原写成裸 `/characters`，缺房间归属路径，2026-07-10 修正）；服务端校验 `phase>=ModuleSelected` |
| `PATCH` | `/rooms/{roomId}/characters/{characterId}` | ★新增：建卡草稿分步增量保存 |
| `POST` | `/rooms/{roomId}/characters/{characterId}/roll-attributes` | ★新增：`attribute_gen_method='roll'` 时，服务端权威计算并写入属性（见 §5.2.2 掷骰原则） |
| `POST` | `/rooms/{roomId}/characters/{characterId}/complete` | ★新增：建卡完成校验（职业点分配/信用评级区间/点数预算），失败返回字段级错误 |
| `GET` | `/users/me/rooms` | ★新增：我参与过的房间列表（🔒 需登录），复盘的入口 |
| `GET` | `/rooms/{roomId}/summary` | ★新增：复盘摘要（`room_summaries`），`game.ended` 后异步生成，未生成完返回 `202 pending` |
| `GET` | `/rooms/{roomId}/replay` | 复盘：拉取 EventLog（供赛后回看）——2026-07-11 拍板返回**全部事件，包括 `visibility:'private'`**（复盘阶段悬念不成立，跟游玩中「不泄底」铁律不冲突） |
...
| 方向 | 事件 | payload 字段 | 说明 |
|---|---|---|---|
| C→S | `room.join` | `{ roomCode, nickname }` | 成功后服务端把 `{roomId, playerId}` **绑定在这条 WS 连接的会话上**（不是让客户端在后续消息里自报），见 §2.1 通信铁律二。🔴 **2026-07-11 新增前提**：WS 建连时必须先带账号 token 校验通过（ADR-16 登录强制前提），才允许发 `room.join`——这是账号级鉴权（"你是谁"），跟 `room.join` 成功后下发的 `reconnectToken`（房间会话级，"你在这个房间里的哪个位置"）是两层不同凭证 |
...
| C→S | `check.manual` ★ | `{ skill }` | 🆕 2026-07-11：路径B入口，玩家主动选技能发起检定（不经过 `action.submit`/`IntentParser`），服务端按角色当前技能值现算 `target`（无 Checkpoint 时按 `regular` 难度），随后走跟路径A相同的 `check.request`/`check.roll`/`check.result`；详见 [[AI编排架构详细设计]] §1.6/§1.7 |
| C→S | `check.roll` ★ | `{ checkId }` | 🔧 2026-07-11 改用 `checkId`（原 `checkpointId?`）：玩家完成"摇骰子"这个物理动作时发送，服务端此时才计算随机数（见下方"服务端权威掷骰"）；`checkId` 是服务端在 `check.request` 里签发的一次性关联令牌，路径A/B统一用它对话，不再依赖只有路径A才有的 `checkpointId` |
| C→S | `san.check.roll` | `{ checkId }` | 🆕 2026-07-11：理智检定摇骰，跟 `check.roll` 同构，详见 [[AI编排架构详细设计]] §2.3 |
| C→S | `luck.spend` / `luck.skip` | `{ checkId, amount? }` | 🆕 2026-07-11：幸运消耗决策窗口——技能检定用于改变成功/失败，理智检定用于减半损失，两套语义不同，`amount` 上限由服务端算好告知、不信任客户端自报；详见 [[AI编排架构详细设计]] §2.3（附 COC7 官方规则调研出处） |
| C→S | `voice.chunk` | `{ seq: number, audio: base64, format: 'opus', isFinal: boolean }` | 见 5.2.3 |
| C→S | `qa.ask` | `{ question }` | 🆕 2026-07-11：QA 答疑独立入口，不经过 `action.submit`/`IntentParser`，天然不受回合门控（不是 `action.submit`）；详见 [[AI编排架构详细设计]] §四 |
...
| S→C 场景 ★ | `narration.push` | `{ sceneId, text, streaming: boolean, seq, done? }` | 旁白，流式分片推送；🔧 2026-07-11 补 `done`——最后一个分片为 `true`，对应底层模型流关闭的机械事实，客户端据此收起"输入中"指示器；听众=当前 `location===sceneId` 的玩家集合（Gateway 经 `GameStateRepo.load()` 取到的 `GameState` 现算，不是固定订阅列表——角色随时可能移动） |
...
| S→C 私密 | `check.request` | `{ checkId, skill, target, checkpointId? }` | 🔧 补 `checkId`（必带，路径A/B统一）；`checkpointId` 仅路径A（命中 Checkpoint）时有值。要求该玩家的客户端展示掷骰 UI；`hidden=true` 的检定**连这个事件也不下发**（2026-07-11 明确，不只是 `check.result`——否则玩家看到"展示掷骰UI"这个动作本身就已经暴露"这里有检定"，暗骰的意义就没了） |
| S→C 私密 | `check.result` | `{ checkId, roll, target, success }` | 🔧 补 `checkId`。`hidden=true` 的检定不下发此事件（抹去，见 §4.3.1 `Checkpoint.hidden`）。命中 Checkpoint（`resolutionKind==='checkpoint'`）时 `appliedOutcome` 里的 `grantsEntityIds`/`sanLoss`/`sceneTransition` 依次生效；未命中预设内容（`resolutionKind==='improvised'`，见 [[AI编排架构详细设计]] §1.7）时骰子结果照常下发，但不执行任何预设效果，只作为 Narrator 的叙事素材 |
| S→C 私密 | `check.result.preliminary` | `{ checkId, roll, target, success: false, maxLuckSpend, currentLuck }` | 🆕 2026-07-11：仅技能检定失败时发，给玩家幸运决策窗口；玩家回应 `luck.spend`/`luck.skip` 后才发最终 `check.result` |
| S→C 私密 | `san.check.request` / `san.check.result(.preliminary)` | 结构类似 `check.request`/`check.result`，但 `success` 不因幸运改变、幸运只减 `sanLossFinal` | 🆕 2026-07-11：理智检定独立协议族，字段形状与调研出处见 [[AI编排架构详细设计]] §2.3；`SanTrigger` 其余五种非 `check` 形态（`flat`/`direct`/`max_reduce`/`gain`/`capped`）不走这套协议，无摇骰环节，由 `RulesEngine` 内部直接生效，靠 `narration.push` + 下次 `view.private` 体现 |
| S→C 私密 | `qa.answer` | `{ questionId, text, streaming, seq, done }` | 🆕 2026-07-11：QA 答疑回应，仅提问者可见；`done` 同 `narration.push` |
| S→C 私密 | `clue.granted` | `{ entity: Entity }` | Entity 类型见 §4.3.1（原 `Clue`，随合并改名） |
| S→C 私密 | `error` | `{ code, message, relatedEventId? }` | 错误码见 5.4 |

> **🎲 服务端权威掷骰（2026-07-11 新增原则）**：不管是建卡属性掷骰（§5.1 `.../roll-attributes`）还是这里的技能检定，骰子结果一律由服务端计算，客户端只展示、不自己算完再上报——`check.request` 打开 UI 后，玩家做出摇骰动作触发 `check.roll`，服务端此时才算随机数，回传 `check.result`。HP/SAN 这类机制性数值变化同理必须由 `RulesEngine` 确定性计算（`CheckOutcome.sanLoss` 本身是骰子表达式如 `"1d4"`，不是自由文本），AI 只读变化前后的状态生成叙事，不决定数字本身。理智检定（挂 `SanTrigger`，性质是"损失多少"而非"过不过"）与幸运消耗（"先看点数再决定花不花幸运"的中间决策窗口）的具体协议形状尚未展开，见文末待办。**2026-07-11 AI 编排详细设计第一轮**：`check.roll`/`check.request`/`check.result` 统一改用 `checkId` 关联（不再依赖只有路径A才有的 `checkpointId`），新增路径B入口 `check.manual`；"没有预设 Checkpoint 命中"时怎么处理（AI 临场判定四层分级）与 `CheckOutcome`/`CheckRollResult` 命名去重详见 [[AI编排架构详细设计]] §一。
...
| 不变量 | 怎么强制 |
|---|---|
| **不泄底** | LLM 函数签名只收 `PlayerView`，**类型上拿不到 `GameState`**——泄底成为编译期不可能（唯一例外 `SummaryGenerator`，见 §4.5——游戏结束后悬念不成立，呼应 `GET /replay` 全事件可见的既有边界，不算违反） |
...
```typescript
type ModelRole = 'intent' | 'narrator' | 'npc' | 'qa' | 'summarizer';  // 🆕 2026-07-11 补 'summarizer'（复盘摘要生成，见 §4.5 SummaryGenerator/[[AI编排架构详细设计]] §五）——非流式/大上下文/一次性，特性跟 narrator 不同，配置独立但 persona 复用 narrator 人格

interface ModelRoleConfig {
  role: ModelRole;
  provider: string;        // 具体供应商标识，底层调用据此选实现
  model: string;           // 具体型号
  params?: { temperature?: number; maxTokens?: number };
}
```
每个 `ModelRole` 独立配置供应商+型号+参数，运行时按 `role` 查表选实现——换模型、混用多家厂商、A/B 测试新模型，都只改配置，不动编排逻辑。供应商差异（function-calling 格式、JSON 模式）由 §4.5 里各角色接口（`IntentParser`/`Narrator`/`QAResponder`/`SanJudge`/`SummaryGenerator`）自己内部按 `role` 查 `ModelRoleConfig` 后调用对应供应商，对编排层暴露的仍是各自那个统一签名。

> **🔧 2026-07-12 补记（文档自查发现）**：本节此前还定义过一个 `ModelAdapter` 接口（只有 `parseIntent`/`narrate` 两个方法，`narrate` 用 `ModelRole` 类型），是 2026-07-09 早期设计，没跟上 2026-07-11 §4.5 新增 `QAResponder`/`SanJudge`/`SummaryGenerator` 三个角色专属接口这轮迭代——`ModelAdapter` 一直停留在只覆盖两个角色的旧版本，且 `narrate` 用的 `ModelRole`（5 值）与 §4.5 `Narrator.narrate()` 用的 `Persona`（3 值）也没对齐，是两轮设计之间遗留的撞车。**按"跟随最新迭代"原则处理**：`ModelAdapter` 本身已被 §4.5 五个角色各自的专属接口取代，予以移除；`ModelRole`/`ModelRoleConfig` 这两个配置层类型仍然有效保留（config 层的角色划分本身没有过时，过时的只是"用一个统一 `ModelAdapter` 接口封装底层调用"这个已被更细粒度接口取代的设计）。
...
每次角色接口（`IntentParser`/`Narrator`/`QAResponder`/`SanJudge`/`SummaryGenerator`）内部的模型调用记录：`role` / `provider+model` / `input_tokens` / `output_tokens` / `latency` / `估算成本`，追加进 EventLog 或专用指标表，按房间/场次汇总——这是 §七「可观测性」在 AI 编排层的具体落点，让「一局到底花了多少钱」可查。
...
> **落地时的目录结构参考**（原 MS1 §六，仍是有效参考，但示例是 TS 全栈时期写的，落地时按 ADR-9 把 `core`/`server` 换成 Python 包结构，`web` 不变）：`packages/core/{content,rules,view,ai,orchestrator}` + `packages/server`（Gateway，暴露 §五 REST/WS）+ `packages/web`（手机端）。各模块 MS1 交付状态：Content=内置 1 个最小模组；Rules/Check/View=真实现；Intent/Narrator=先桩后接模型（各自按 §6.3 `ModelRoleConfig` 查表选供应商/型号）；Server/Web=最小可回显。⚠️ 依 CLAUDE.md「不主动建脚手架」——这仍是**计划**，建工程需用户明确点头进入落地。
...
| **ADR-15** | `users` 表由占位转正，字段用**自定义账号+密码**，作为 `reconnectToken` 机制的补充层（2026-07-10 定，§4.4.8） | 维持纯设备本地 `reconnectToken`（不落账号）；或接微信 OpenID / 短信验证码登录 | 用户明确要求支持「关 App 后换设备/重装找回同一局游戏」——`reconnectToken` 本质绑定单一设备/浏览器（存 `localStorage`），换设备后这份凭证不存在，无法满足需求，必须有一个不依赖设备的身份凭证，即账号。三选一（匿名设备 ID／微信 OpenID／自定义账号密码）中，团队当前无已确认的微信开发者资质，产品是 PWA（见 `vite-plugin-pwa` 配置）而非微信生态原生应用，短信网关又引入额外第三方依赖；选自定义账号密码：零第三方依赖、8 周周期内开发成本最可控。**账号最初定为可选层**——不强制所有房间参与者注册，游客仍可只用房间码 + `reconnectToken` 一次性加入；**⚠️ 这条"可选层"结论已被下面 ADR-16 推翻，本行保留作历史记录，不代表当前状态** |
| **ADR-16** | 账号登录由「可选层」升级为**玩游戏的硬性前提**（2026-07-11 定，API 详细设计阶段，§4.4.8） | 维持 ADR-15 的「可选层」——账号只用于跨设备找回，不影响能不能玩 | **推翻 ADR-15 的「不强制访客注册」结论**。产品诉求明确「每个玩家都要能复盘、掉线后能重新进来」——`reconnectToken` 只能解决同设备内的会话级重连，解决不了跨设备/隔一段时间后的找回，要满足这个诉求，每个真人玩家必须有服务端持久身份。`players.user_id`/`rooms.host_user_id` 从可空外键改为非空（`is_ai=true` 的 AI 队友行例外）。注册与加入房间故意不合并成一步——账号密码必须玩家本人主动设置，系统代填的话换设备就登不回来，账号形同虚设。浏览游戏/模组目录不受影响，仍不需要登录。详见 [[2026-07-11 账号强制登录-推翻不强制访客注册]] |
| **ADR-17** | DB 引擎选定 **PostgreSQL**（2026-07-11 定，§4.4） | MySQL；SQLite | 项目是「传统后端任务 + AI 应用」的结合——PostgreSQL 既能承担关系型数据的常规需求，又能通过 `pgvector` 扩展支持向量存储，为未来可能的语义检索/RAG 场景（如模组导入解析、AI 编排的上下文检索）预留同一个引擎内的能力，不需要额外引入一套专用向量数据库。**ADR-13 的 PostgreSQL RLS 第二道防线**（此前是"若选定该引擎"的条件项）现在条件已满足，是否启用待定，见待办 |
...
| 数据 | 房间/车卡/状态/日志/内置模组落库、**账号体系**(自定义账号密码，**强制前提**，2026-07-11 起，§4.4.8/ADR-16) | 模组**导入**打通(D2) |
...
- [x] ~~DB 引擎选型~~ → **已完成（2026-07-11）**：选定 **PostgreSQL**（新增 ADR-17）——项目是「传统后端任务+AI应用」结合，PostgreSQL 通过 `pgvector` 扩展可在同一引擎内支持向量存储，兼顾常规关系型需求与未来语义检索场景。ADR-13 的 RLS 第二道防线是否启用待定
- [x] ~~卡关引导策略~~（核心且单路径检定点玩家反复失败时如何主动提示）→ **已完成（2026-07-11，AI 编排详细设计第六轮）**：`turnsSinceLastCoreDiscovery` 现查 EventLog、不持久化 + 三级 policy（叙事加权→NPC/环境主动递话→半直给），阈值是 World 级配置参数非运行时状态；详见 [[AI编排架构详细设计]] §六
- [x] ~~`users` 表由占位转正~~ → **已完成**（2026-07-10：自定义账号+密码，补「换设备/重装找回」场景，与 `reconnectToken` 叠加而非替代；`players.user_id`/`rooms.host_user_id` 两处新增外键，ER 图/§4.2/§十/新增 §4.4.8/新增 ADR-15 同步更新，见 [[2026-07-10 账号体系设计-users表转正]]）。**⚠️ 当时定的"可选层"已于 2026-07-11 被 ADR-16 推翻，见下一行**
- [x] ~~账号登录由可选层升级为强制前提~~ → **已完成（2026-07-11，API 详细设计阶段）**：新增 ADR-16，`players.user_id`/`rooms.host_user_id` 改非空，详见 §4.4.8、[[2026-07-11 账号强制登录-推翻不强制访客注册]]
...
- [x] ~~API 详细设计~~ → **六块已过完一轮（2026-07-11）**：模组导入(异步化+失败兜底)、账号登录(强制前提)、大厅与房间(创建时机+幂等选模组)、建卡(骨架+服务端权威掷骰原则)、WS事件对齐(同步Entity/scene档/补room.rejoin等)、复盘统计(全事件可见+我的房间列表)。完整请求/响应示例见 [[API接口对齐规范]]，本文档只同步了端点一览表和关键原则，不是全量搬运
- [x] ~~§4.4 缺 games/game_systems/user_sessions/room_summaries/room_sessions/module_import_jobs 等新表索引~~ → **已完成（2026-07-11）**：新增 §4.4.9，只列表名+一句话+出处指路，不复制字段细节，完整定义仍以 [[前端demo数据表设计]] 为准（用户明确要求维持三份文档、不整体合并，避免 master 冗余）
- [ ] **协议细节明确推迟**（API 详细设计阶段发现，暂不深挖）：~~SAN 检定独立协议~~、~~幸运消耗的两段式协议时序~~、~~`check.roll` 具体字段形状~~、~~没有预设内容时 AI 临场判定的结果怎么走通道~~——**四条均已在 AI 编排详细设计第一/二轮解决**（见下条）；只剩 `PlayerView` 补全 HP/MP/幸运（现在只有 `visibleSan`）待做，这条较轻量，找机会顺手补
- [x] ~~AI 编排架构详细设计~~（跟数据表/API设计同等深度）→ **六轮全部完成（2026-07-11）**：①意图解析与检定链路边界；②SAN 触发与幸运消耗协议；③旁白与 NPC 对话的边界；④QA 答疑；⑤复盘摘要生成机制；⑥卡关引导策略。新增 [[AI编排架构详细设计]]（`AI编排/`），§4.3.0/§4.5/§5.2.2 已同步。
- [x] ~~六轮完成后的收尾审计~~ → **已完成（2026-07-11）**：对照 [[架构设计参考 1]] 方法论逐一核对 API/数据表/协议三份文档，发现并处理：①[[API接口对齐规范]] §三严重过时（缺六轮新增的全部事件字段）→ 已全量回顾同步；②[[前端demo数据表设计]] `check_results` 缺 `check_kind` 判别理智检定 vs 技能检定 → 已补；③本节"不泄底"不变量描述过于绝对，未标注 `SummaryGenerator` 例外 → 已加脚注。`check_results.luck_spent`/`room_summaries` schema 核对无冲突
- [ ] 组内评审本多视图架构；确认后建空骨架（多端版 skeleton，目录参考见 §八；用户明确参照 techcamp.wiki `02-架构设计规范`"总架构先行/代码即架构"理念——骨架产出直接是代码本身，模块内部实现留空，系统架构由人主导设计，不是 AI 读文档自由生成）——落地时 §4.5 接口契约换算为 Python/Pydantic + ORM 模型，对齐 §4.4 表结构
