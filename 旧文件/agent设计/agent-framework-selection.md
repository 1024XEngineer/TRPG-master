...
按照设计，系统由 **两个 Agent 系统** 和 **一个确定性引擎** 组成：
...
**核心优势总结**：项目的核心接口（ActionPlan、ConfirmedOutcome、ModuleContent）在设计文档中已是强类型结构，直接用 Pydantic Model 实现后，PydanticAI 的 `result_type` 一行代码即可消除 LLM 输出解析这整类 bug。
...
| 状态机契合度 | ★★★★★ | 同上。 |
| 结构化 JSON 契合度 | ★★★★★ | 项目的每个 JSON 接口（ModuleContent、ActionPlan、ConfirmedOutcome、Event）在设计文档中已是强类型结构，用 Pydantic Model 实现后，PydanticAI 验证 LLM 侧，Pydantic 验证引擎侧，同一个类型系统贯穿全栈。 |
...
系统需要记忆——玩家调查过书架、NPC 说过什么、钥匙已经找到——这些历史信息 Planner 和 Narrator 都必须知道。但记忆的实现方式不是向量数据库或 LangChain Memory 模块，而是从 EventLog 和物化视图中按需组装。

```text
┌──────────────────────────────────────────────────────┐
│             Memory = EventLog + 物化视图               │
│                                                       │
│  游戏事实记忆（Planner 需要）:                           │
│    · entity_states: 当前所有实体的状态                  │
│      （bookshelf.key_found = true）                    │
│    · character 物化视图: HP/SAN/装备/位置               │
│    · 最近 N 条 player_visible events:                  │
│      "第 3 回合：玩家在书架后发现了一把小钥匙"           │
│      "第 4 回合：管家说'那柜子很久没打开了'"             │
│    → 从 PG events 表按 (room_id, ts desc) 查询        │
│    → ContextAssembler 格式化为文本注入 Planner prompt   │
│                                                       │
│  叙事连贯性记忆（Narrator 需要）:                         │
│    · 最近 M 回合的叙事文本                              │
│    · 用于保持语气、风格、NPC 人设连贯                    │
│    → 从 EventLog 中取 type=narration 的事件            │
│    → 作为 few-shot 风格参考注入 Narrator prompt         │
│                                                       │
│  硬状态记忆（引擎需要）:                                  │
│    · 完整 GameState（entity_states + characters）      │
│    · 物化视图，可从 events 全量回放重建                  │
│    · 断线重连：回放 events 恢复到断线前状态              │
│    · MVP: 直接从 PG 读取（单房间 ~19KB）                │
│    · 中期: Redis 缓存热状态                             │
└──────────────────────────────────────────────────────┘
```

**为什么用 EventLog 做记忆而不是向量数据库：**

1. 游戏记忆是精确事实，不是语义相似。"玩家有没有钥匙"是一个 bool，不需要 embedding。
2. 向量数据库适合"找和这个查询最相关的段落"，游戏需要的是"最近 5 回合发生了什么"——按时间排序即可。
3. EventLog 天然可审计——每个事实都有时间戳和 cause，调试时能回答"这个状态是什么时候、因为什么变的"。
4. 回合数有限。一场 CoC 跑团通常 50-200 回合，全部 player_visible events 序列化进 prompt 完全装得下（Claude 200K 上下文足够容纳整场游戏的叙事历史）。

**注意：这和"不用 Memory"是两回事。** 系统用 EventLog 实现记忆，而不是用向量数据库或 LangChain Memory 类。ContextAssembler 的职责就是从 EventLog 中提取最近回合的事实和叙事，注入 LLM 上下文。
...
未来可能考虑增加的功能：NPC Agent、Combat Agent、Investigation Agent、Emotion Agent。
