...
| 粗粒度导入 Agent | `packages/core/moduleimport/agent.py` | 已有 `parse()`、`validate_and_ingest()` 和 `ModulePackDraft` 桩（即现在的 `ModuleDraft`，旧称）；内部 Parser、确定性校验、Reviewer、ReviewReport 和批准流程尚未实现 |
